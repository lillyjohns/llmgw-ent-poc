...
export async function orgNew(body: any): Promise<AdminResponse> {
  const orgId = body.org_id || generateOrgId();
  ...
  const record = {
    ...
    SK: 'META',
    org_id: orgId,
    org_alias: body.org_alias || orgId,
...
export async function orgInfo(queryParams: Record<string, string>): Promise<AdminResponse> {
...
export async function orgUpdate(body: any): Promise<AdminResponse> {
...
export async function orgDelete(queryParams: Record<string, string>): Promise<AdminResponse> {
  const orgId = queryParams.org_id;
  if (!orgId) {
    return jsonResponse(400, { error: { message: 'Missing required query parameter: org_id', type: 'invalid_request' } });
  }
...
export async function orgMembers(queryParams: Record<string, string>): Promise<AdminResponse> {
  const orgId = queryParams.org_id;
  if (!orgId) {
    return jsonResponse(400, { error: { message: 'Missing required query parameter: org_id', type: 'invalid_request' } });
  }
  ...
  const users = (userResult.Items || [])
    .filter((item) => {
      const userTeamIds: string[] = item.team_ids || [];
