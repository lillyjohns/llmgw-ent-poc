...
async function keyGenerate(body: any): Promise<AdminResponse> {
  const apiKey = generateApiKey();
  const keyHash = hashKey(apiKey);
  const keyId = generateKeyId();
  const now = new Date().toISOString();
  const expiresAt = computeExpiry(body.duration);
  const budgetResetAt = computeExpiry(body.budget_duration);
  ...
  const record: KeyRecord = {
    ...
    key_id: keyId,
    key_hash: keyHash,
    key_alias: body.key_alias || undefined,
    team_id: body.team_id || undefined,
    org_id: body.org_id || undefined,
    models: body.models || undefined,
    max_budget: body.max_budget || undefined,
    spend: 0,
    rpm_limit: body.rpm_limit || undefined,
    tpm_limit: body.tpm_limit || undefined,
    metadata: body.metadata || undefined,
    blocked: false,
    created_at: now,
    expires_at: expiresAt,
    budget_duration: body.budget_duration || undefined,
...
async function keyInfo(queryParams: Record<string, string>): Promise<AdminResponse> {
...
async function keyUpdate(body: any): Promise<AdminResponse> {
  const key = body.key;
  if (!key) {
  ...
  for (const [inputKey, dbField] of Object.entries(allowedFields)) {
    if (body[inputKey] !== undefined) {
      ...
      if (inputKey === 'duration') {
        const expiry = computeExpiry(value);
        if (!expiry) continue;
        value = expiry;
      }
...
async function keyDelete(body: any): Promise<AdminResponse> {
  const key = body.key;
  if (!key) {
    return jsonResponse(400, { error: 'Missing required field: key' });
  }
...
async function keyList(queryParams: Record<string, string>): Promise<AdminResponse> {
...
async function keyBlock(body: any): Promise<AdminResponse> {
  const key = body.key;
  if (!key) {
    return jsonResponse(400, { error: 'Missing required field: key' });
  }
...
async function keyUnblock(body: any): Promise<AdminResponse> {
  const key = body.key;
  if (!key) {
    return jsonResponse(400, { error: 'Missing required field: key' });
  }
...
async function teamNew(body: any): Promise<AdminResponse> {
  ...
  const record: TeamRecord = {
    ...
    SK: 'META',
    team_id: teamId,
    team_alias: body.team_alias || teamId,
    org_id: body.org_id || undefined,
    max_budget: body.max_budget || undefined,
    spend: 0,
    budget_duration: body.budget_duration || undefined,
...
async function teamInfo(queryParams: Record<string, string>): Promise<AdminResponse> {
  const teamId = queryParams.team_id;
  if (!teamId) {
...
async function teamUpdate(body: any): Promise<AdminResponse> {
  ...
  if (body.budget_duration) {
    const resetAt = computeExpiry(body.budget_duration);
    if (resetAt) {
...
async function teamDelete(body: any): Promise<AdminResponse> {
  const teamId = body.team_id;
  if (!teamId) {
    return jsonResponse(400, { error: 'Missing required field: team_id' });
  }
...
async function modelAdd(body: any): Promise<AdminResponse> {
  const modelName = body.model_name;
  if (!modelName) {
    return jsonResponse(400, { error: 'Missing required field: model_name' });
  }

  const modelId = body.model_id || modelName.replace(/[^a-zA-Z0-9-_]/g, '-');
  const now = new Date().toISOString();
  ...
  const record = {
    ...
    input_cost_per_1k: body.input_cost_per_1k || undefined,
    output_cost_per_1k: body.output_cost_per_1k || undefined,
...
async function modelDelete(body: any): Promise<AdminResponse> {
...
async function spendKeys(queryParams: Record<string, string>): Promise<AdminResponse> {
  const result = await docClient.send(new ScanCommand({
    TableName: TABLE_NAME,
    FilterExpression: 'begins_with(PK, :prefix)',
...
async function spendReset(queryParams: Record<string, string>): Promise<AdminResponse> {
  const key = queryParams.key;
  const teamId = queryParams.team_id;
