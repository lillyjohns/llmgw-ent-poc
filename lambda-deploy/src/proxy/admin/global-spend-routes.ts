...
export async function globalSpendReport(queryParams: Record<string, string>): Promise<AdminResponse> {
...
export async function globalActivity(queryParams: Record<string, string>): Promise<AdminResponse> {
  ...
  const activityMap: Record<string, ActivityBucket> = {};
  for (const item of result.Items || []) {
