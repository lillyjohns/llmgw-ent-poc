...
export class DynamoKeyValidator {
    ...
    try {
      ...
      if (result.Item) {
        return {
          ...
          key_alias: result.Item.key_alias,
        };
      }
    } catch (err: any) {
   ...
   */
  async incrementSpend(apiKey: string, amount: number) {
    const keyHash = this.hashKey(apiKey);
    try {
   ...
   */
  async checkBudgetPreRequest(apiKey: string, estimatedCost: number) {
    const keyHash = this.hashKey(apiKey);
    try {
      const result = await docClient.send(new GetCommand({
        TableName: TABLE_NAME,
      ...
      if (!result.Item || !result.Item.max_budget) {
        return { allowed: true, remaining: Infinity, maxBudget: undefined };
      }
   ...
   */
  async seedKey(apiKey: string, info: Partial<KeyInfo>) {
    const keyHash = this.hashKey(apiKey);
